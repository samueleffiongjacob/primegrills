...
const PromotionCard: React.FC<PromotionProps> = ({ promotionItems }) => {
  ...
  if (loading) {
    return (
      <div className="animate-pulse flex justify-center items-center mb-10">
        <div className="w-full">
          {[...Array(1)].map((_, index) => (
            <div
              key={index}
              className="flex items-center justify-between rounded-3xl p-8 md:h-54 relative bg-gray-300 overflow-hidden mb-6"
            >
              <div className="absolute -top-3 -left-3 bg-gray-400 px-4 py-1 rounded-full w-24 h-6"></div>
              ...
              <div className="w-24 md:w-52 h-24 md:h-52 bg-gray-400 rounded-full"></div>
            </div>
          ))}
        </div>
      </div>
